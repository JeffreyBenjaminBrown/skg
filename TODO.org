...
*** prompt
Currently there are these fields in the TrueNode type:
```
  pub cycle          : bool,
  pub stats          : TrueNodeStats,
```
I would like to replace them with these two fields:
```
  pub graphStats : GraphNodeStats
  pub viewStats : ViewNodeStats
```
where
  *** these become graph stats
      pub numContainers: Option<usize>,
      pub numContents: Option<usize>,
      pub numLinksIn: Option<usize>,
  *** these become view stats
      pub cyycle : bool
      pub parentIsContainer: bool,
      pub parentIsContent: bool,

In thoughts/divide_stats.org, please write a plan for how to refactor the code to do this. The refactor should cover both server/ (the server) and elisp/ (the client).
** TODO ? put parentIgnroes in ViewNodeStats
** TODO most natural
*** for subscribee cols
