...
* priorities
** ONGOING overhaul metadata view
*** metadata view should show even fields absent in the metadata
*** when making a new root, present with source options
    Just do it in emacs.
    Jump to metadata view, cycling through source options
      with a shortcut for cycling explained at the top.
** validation should permit ghosts to have same-id siblings
*** the error message
 ```
   ...
   ** Error 1
   ...
   - TrueNode's non-ignored TrueNode children must be unique (no two sharing the same ID).
 ```
*** how to fix the code
    Currently, the part of the validation code that
    prevents siblings from having duplicate IDs
    excludes (I believe) 'parentIgnores' nodes,
    but does not exclude phantom nodes ('removed' or 'removedHere').
    It should exclude those, too.
