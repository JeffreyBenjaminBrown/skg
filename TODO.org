* ONGOING refactor for skgnodemap
** TODO reviewing the diff against main
*** RESUME AT
    a call to
      ??
    in
      tools/refactor_conversion_site_prioritizer.py
*** new functions
...
*** add type signatures everywhere
    this makes it easy to search for types,
    for more refactors
*** everything that inserts into a SkgNodeMap
    should first try looking that thing up in it
*** simplify completeAliasCol using helpers
*** remove unused arguments from functions
    e.g. pub async fn completeAliasCol
*** fix internal imports
I'm finding more internal imports, like this:
```
+  forest        : &mut ego_tree::Tree<OrgNode>, // "forest" = tree with ForestRoot
```
Please restrict that kind of notation to the 'use' statements at the top of each
file. (I suggest going through the git diff to find those -- again taking the diff
against main.)
** missed some
   remove_completed_view_request
** keep this up to date
   refactor-ongoing/functions.org
** once it's not globally needed, restrict PairTree and NodePair
   via 'super' or, better even if isolated enough, local
** DONE use call graph
   Hey Claude would it help if you could see a text representation of the call graph? I don't know if leafward or rootward functions are easier to convert, but either way you could see where they lie by looking at (for instance) tools/introspect/call-graph/output/main.org. You can regenerate those files with 'cargo run -p call-graph --bin call-graph-regen'. Maybe crossing that data with functions.org would give you a good sense of which is the easiest stuff left.
   (To do that I think you might want to automate the merge of those two data sets -- write something that determines, for each as-yet-unconverted function in functions.org, the number of its descendents in the call graph that still need conversion, and how many of its ancestors do. But I don't know whether that information would actually be useful to you. So don't do that; just think about the idea and respond, please.)
** DONE functions miscategorized as pairless
   But also note that,
   Under the headline 'new-functions-without-correspondences'
   in refactor-ongoing/functions.org,
   lots of '{name}_in_orgtree' functions correspond to '{name}' ones.
   That is, they are miscategorized -- they appear to be v2 versions
   to replace v1 versions.
** DONE faster
   I feel like this is going slower than normal for you. Do you think there's any way you could organize the plans in refactor-ongoing/ to speed it up? Maybe think harder about plans before doing it? I don't know -- you tell me if something like that is possible. Otherwise continue.
* instructions when asking for a refactor
...
* two kinds of stats
** the idea
   One regards the view, the other the graph.
   Graph stats can be computed after save, before updating the view.
   These would include link, alias, subscribe and override rels.
** these should be graph stats
   pub numContainers: Option<usize>,
   pub numContents: Option<usize>,
   pub numLinksIn: Option<usize>,
** these should be view stats
   pub cyycle : bool
   pub parentIsContainer: bool,
   pub parentIsContent: bool,
* understand it
** AWAITS What's the logic behind this comment?
*** dunno, but it will be done
    once I've finished refactoring to use (org tree, skgnodemap)
    rather than pairtree.
*** this comment
...
*** for subscribee cols
    ...
    but if the user deletes that from the view, fine.
...
** makeIndefinitiveAndClobber doesn't sound right
   It sets indef and body but not title.
   Something else does all three.
