...
* Rename BufferRoot to Buffer.
  And permit it to have no children.
...
****** Add comment: None should have a title but Alias.
       Encoding this as a blanket statement would be awkward;
       instead the code should state independent body treatments
       for each Scaffold.
***** BufferRoot
      Should be the root.
      Add comment: Needs no children!
***** Alias
      title, no body, no children, aliasCol parent
***** AliasCol
      No title.
      Only Alias children.
      Parent is a TrueNode.
***** HiddenInSubscribeeCol
      No title, body.
      Parent is Subscribee.
      Children are only TrueNodes.
***** SubscribeeCol
      Parent is TrueNode.
      Children are only TrueNodes or HiddenOutsideOfSubscribeeCol.
***** TextChanged
      No title. How to display this should be left to Emacs.
***** TrueNode
