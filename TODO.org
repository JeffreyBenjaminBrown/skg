* rename save types
** rename SaveInstruction -> DefineOneNode
** redefine DefineOneNode =
   Save (SkgNode) | Delete (SkgNode)
** rename MergeInstructionTriple -> Merge
   should become three DefineOneNodes:
   two saves and a delete
** aborted
*** enum save = oneNode | merge
**** idea
    SaveInstruction -> Save::OneNode
    MergeInstruction -> Save::Merge
**** request
  Let's replace this definition:

    pub type SaveInstruction = (SkgNode, NonMerge_NodeAction);

  and the types it is defined in terms of (maybe recursively)
  to this:
  ...
  (which implies we are, in addition to restructuring, renaming MergeInstructionTriple to simply Merge).

  The new Merge type should have the three MergeInstructionTriple fields as before, but now they contain Saves.

  We get rid of NonMerge_NodeAction.

  Preserve all comments in carrying this out.
* Why mod files are better.
  This way, if there is parent code that relies on the code in (name)/,
  it can be named (name).rs, and a reader is likely to read it.
  The alternative, that (name).rs is usually a boring mod file,
  leads at least this reader to likely skip reading (name).rs,
