...
*** ONGOING functions to delete
    KEEP THE SAME SPACING
**** a table of uses
***** ONGOING is_scaffold
***** AWAITS clear_body
Next, clear_body. Note that in any context in which we are setting the body, we should be expecting a TrueNode, so error otherwise.
***** set_cycle
***** is_indefinitive() -- used maybe 13 times
***** view_requests_mut
***** set_title
***** set_indefinitive
***** set_edit_request
...
***** set_num_contents
***** set_num_links_in
***** set_parent_is_content
***** set_effect_on_parent
***** set_id
**** impl OrgNode {
*** TrueNode should bundle the id and source in a single Opt
