* complete_preorder_for_one_node calls complete_truenode
  so it's not really for one node
* is this logic duplicated anywhere?
...
* rename things in save_buffer
* ? rerender_collateral_views v. rerender_view
  e.g. complete_collateral_view calls rerender_view,
  which seems backward
* [[id:e6fff78e-0fc6-47e9-b444-06718a88be12][replace ( *token *) with (token)]]
