...
*** rename indefinitive -> definesNeitherTextNorContents
*** comment : 'indefinitive' should go wherever 'repeated' does.
    But not the reverse.
*** comment : 'indef' is about the relationship to children.
** comment : ViewData combines what to display with how to display.
   folded and focused are instructions to emacs,
   the rest, glyphs
** merge the two Save errors
** rename TextLink -> TextLink
