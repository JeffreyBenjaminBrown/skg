...
** / definition & example /
*** Write a function doToPairTreeNode
**** Hides all the node id fetching ogic
**** Can take a lambda
** Maybe do it lazily, only where I want to be more readable.
** Maybe do it to all boilerplate.
...
* TODO BLOCKER : maybe_add_subscribee_col needs a lot of work
  It's so complex I can't see past it.
** Subdivide it.
...
* simple refactors
** make NodePair a type, not an alias
   Currently, pub type NodePair = (Option<SkgNode>, OrgNode).
   It would be better to use explicit field names
     'mskgnode' ('m' for 'maybe') and 'orgnode'.
