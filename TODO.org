* TODO RESUME READING / EDITING at
  rust/serve*
    (have not read any of them yet)
* ONGOING I've moved some things.
  I'm collecting 'save' functionality in rust/save.rs and rust/save/.
  Please continue that by moving
    update_index_from_saveinstructions
  to a new file
    rust/save/tantivy.rs
  and then fix all the broken imports so that cargo nextest is happy.
* rename files
In rust/save/, let's rename these files:
...
* break into standalone functions
** format_response_as_sexp
...
* rename wipe_fs_then_create_index_there
  to
    in_fs_wipe_index_then_create_it
* reorder rust/serve/parse_headline_md_sexp.rs
  to put callers first.
  (if there's mutual recursion,
  the order within the mutual recursive functions doesn't matter,
  unless one is clearly an entry point,
  in which case it should be first).
* rename
struct SaveResponse {
  content : String, // rename to 'buffer_content'
* shuffle (*after* reading through!)
** media/{tantivy, typedb, file_io, textlinks}
** function 'cleanup_and_shutdown' to its own file
** group handlers in handler/
   and rename so handler isn't restated
** Are there sexp functions that should be grouped?
* + rust/typedb/init.rs
move
  initialize_typedb_from_nodes
  initialize_tantivy_from_nodes
  initialize_dbs
and probably others to it.
* [[id:7dd8d9fd-8e7f-4719-a547-554eb182beb1][rescue smsn into skg]]
* Appending from indef nodes moght be a misfeature.
* "points" seems a better name than "nodes"
* TODO move buffer_to_save_instructions
  from
    rust/save.rs
  to at the shallowest
    rust/save/buffer_to_orgnodes.rs.
* TODO buy statins, etc.
