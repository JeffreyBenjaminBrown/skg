* dedup completion for primary and collateral views
** the tangle
   Look over update_from_and_rerender_buffer.
   It was structured before I introduced collateral views.
   It needs rearrangement now;
   steps that belong together are separate.
** the shared pipeline
   For collateral views, there's the following.
   For the initial view there should be something similar.
...
* new AI testing strategy
  I specify a feature. AI codes it.
  Commit it *without reviewing*,
    with UNREVIEWED prominent in the commit message.
  I specify tests. AI debugs with the tests.
  *Then* I review the code,
    and maybe squash the last few commits into one.
* Deletedscaffold should record what it used to be
  with a ScaffoldKind field
