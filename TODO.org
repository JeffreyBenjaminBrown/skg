...
*** TODO programmatically search for scope reductions
I asked the AI to do it, but it is clearly fallible, as it had to correct what its subagents did. I could have it check programmatically instead: for each definition, try making it private, and if that fails pub(super), and if that fails leave it pub. And for every pub(super), try making it private. (Ignore pub(crate) -- that distinction doesn't matter to me right now.)
*** TODO wrap temp variables in the scope of what they are used for
*** ONGOING reorg
**** DONE request 1
  Let's:
...
**** TODO RESUME from introspect/code-map/code-map.org
     reading downward
*** ONGOING look for helpers to simplify egotree boilerplate
...
** reading pub async fn completeAliasCol (
 // Validate this is an AliasCol
 let mut parent_skgnode : SkgNode = {
 parent_skgnode.source = parent_source; // BUG ?
