* see noneclobber-might-be-duplicated.org
* reviewing save-buffer
** top
   fn update_from_and_rerender_buffer
     calls : buffer_to_viewnode_forest_and_save_instructions
       calls : viewnode_forest_to_nonmerge_save_instructions
* naive_saveinstructions_from_tree cannot skip SubscribeeCols
* now just make it safe and use it a little
* ? [[id:ddec4e85-1ea5-4ddb-b6ec-d2c3f464dad7][get Claude to use Skg]]
* test buffer update & git diff
** how to test it
*** git diff view and not
*** multiple sources, source ownerships
*** deleted here, deleted, new here, new
*** each kind of node
**** Alias
     { text: String,
...
** some git diff view code I was uncertain about
...
* u
** the non-completion tasks should be moved out of complete_truenode
** ? find all things that return where maybe should error
   e.g. if something isn't found
** this looks like it should error instead
...
** distinguish : RemovedHere != HIddenHere, NewHere != UnhiddenHere
