* NOTE TO READER: Most plans are online.
  e.g. here:
  github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/main/shareable_knowledge_gardens.org
* s, by urgency
** ONGOING futzing the refactor
*** missing comments in types/orgnode.rs
/// The metadata precedes the title in the headline.
/// (Both follow the "bullet", which is a string of asterisks.)
*** rewrite parts of test_orgnode_to_text_no_metadata
    to use OrgNode constructors, for brevity
*** compare_two_orgnode_branches_recursively_modulo_id
    is baroque
*** delete should_be_sourceless
*** merge assign_alias_relation_if_needed into its caller
*** assign_new_id_if_needed should verify it's owned
*** validate_node_and_children
    Rather than two bool arguments,
    take a single ScaffoldKind argument.
*** validate_node_and_children
    Case directly, rather than defining is_alias etc.
*** validate_merge_requests and friends
**** 1
It does this:
```
...
```
That's *terrible* -- it's imagining every Alias will contain an empty String. Find any such test anywhere, and replace it with something that just tests the constructor, not its contents. If a parallel enum ScaffoldKindConstructorName is needed, so be it.
**** 2
Don't worry about alias, aliascol, etc.
Just test whether it's a Scaffold, period.
Actually that whole test seems unneeded -- it can't happen.
*** put all implementations in orgnode.rs after all type dec's
*** merge the Scaffold and ScaffoldKind types
    keeping the first and deleting the second name
*** delete folded and focused from OrgnodeViewData
*** rename
**** OrgnodeViewData -> TrueNodeViewData
*** rewrite mk_*_orgnode
   in terms of mk_orgnode
*** impl OrgNode
**** rename
     Most of those noops should be renamed to append "_if_truenode'.
     has_source should be renamed is_truenode_and_has_source.
     Similarly for cycle.
**** delete
...
+    self . folded }
**** I'm not convinced most of these should exist.
*** what's up with this?
**** the comment
...
**** my concern
     That passage seems to test parsing.
     If the error wouldn't happen there, where would it happen?
*** why did this new RootWithoutSource test need to be added?
...
*** put type signatures here
fn set_metadata_relationships_in_node_recursive (
*** rename is_scaffold
    to has_scaffold_kind
*** seems baroque
fn rebuild_pair_from_disk_mostly_clobbering_the_org (
** type changes
*** delete EffectOnParent
    should only be a bool, parentIgnores, defaulting to false.
...
*** merge OrgnodeRelationships and OrgnodeViewData
    deleting the second name
...
*** break up completeDefinitiveOrgnode
    it's super-long
*** Why do complete_branch_minus_content and completeDefinitiveOrgnode both call maybe_add_subscribeeCol_branch?
...
*** On what Interps can 'completeDefinitiveOrgnode' be validly called?
...
***** omitted from completeAndRestoreNode_collectingViewRequests
***** elaboration
