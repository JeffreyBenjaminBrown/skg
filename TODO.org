* RESUME REVIEW at
  ...
  (reading down in magit)
* reviewing rust/to_org/expand/definitive.rs
** good, no need to unify
   rebuild_pair_from_disk_mostly_clobbering_the_org
** unify?
*** expand::definitive::add_last_generation_and_edit_previous
    with
    render::add_last_generation_truncated
* what calls 'to_org::'
** single_root_view
** completeOrgnodeForest_collectingDefinitiveRequests
** execute_definitive_view_requests
* merge rust/to_org/bfs_shared.rs and rust/to_org/util.rs
* add tests
** collect_generation_ids
   test the new effective root logic
* those indef nodes in a def view request or an initial view should be def if they have no children
* use the same kind of tree for initial and re-rendered views
  This will permit deduplicating a lot of common logic.
* Move 'clobberIndefinitiveOrgnode' to neighor file 'util.rs'
* Request definitive content view at a node
** for indefinitive nodes
** use that logic for building subscription views
   Requesting the subscription view
   unfolds the entire subscribed-to node,
   with what the subscriber hides grouped under 'hides'.
