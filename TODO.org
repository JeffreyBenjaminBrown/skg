...
*** completeAndRestoreNode_collectingViewRequests must handle Subscribees
*** break up completeDefinitiveOrgnode
    it's super-long
*** maybe_add_subscribee_col is impenetrable
    so complex I can't see past it
**** Subdivide it.
**** maybe_add_subscribee_col should integrate existing SubscribeeCol
     if it exists, rather than assuming it is correct
*** Why do complete_branch_minus_content and completeDefinitiveOrgnode both call maybe_add_subscribee_col?
*** After 'collect_view_requests', is there any reason to keep the view requests in the node?
...
*** On what Interps can 'completeDefinitiveOrgnode' be validly called?
    The comment says only Content nodes.
...
*** ONGOING make NodePair a type, not an alias
**** 1
 Wonderful.

 Next, let's make NodePair a type, not an alias. Currently, pub type NodePair = (Option<SkgNode>, OrgNode). (And for all I know there might be the same type with the order swapped somewhere in the code, so please search for both cases, and replace them with this single case:

 It would be better to use a true type, with explicit field names:
      mskgnode : Option<SkgNode> // the 'm' is for 'maybe'
      orgnode  : OrgNode
**** TODO Rewrite clobberIndefinitiveOrgnoe to use them.
*** rename each 'node' to skgnode, mskgnode or orgnode if that's what it is
**** or else write a comment 'name collistion' if that renaming can't be done because something else is already using the name
**** also rename each skgnode to mskgnode if that's what it is
**** orgnode_data is a terrible name
