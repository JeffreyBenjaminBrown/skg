* ONGOING skgnodes-in-memory-request.org
** replace rerender_collateral_view_from_scratch
   which might make the 'root_pids' field obsolete
** extend completion to cover deletion
** test multi-buffer edits
   A cycle a <-> b.
   Open a view from a and a view from b.
   In the view from a, delete b as a child.
   Now the view from b should only have definitive nodes b -> a,
   rather than b -> a -> (indef b).
