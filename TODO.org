* as if anyone else is reading this
** NOTE TO READER: Most plans are not here but online.
   e.g. here:
...
* TODO out of sync with remote
* integration-test that skg-visit-link works
  That was great! Let's integration-test skg-visit-link, to keep that fixed. See tests/integration/ for what integration tests look like. each folder therein is a separate integration test. (Be sure to include the new one in 'tests/integration/run-all-tests.sh' -- actually better yet, just make that script traverse all the subfolders there, so we don't need to remember to update it when adding another integration test.)
  The integration test should be:
  The skg data folder should begin with two .skg files with IDs 'src' and 'dest'. The title of dest is "Dest need not be aware of the link." The title of src is "Src has a [[id:dest][link to dest]]." Neither node has any content or other relationships -- there's just the link, a relationship implicit from the text of one of them. (The server understands it.)
  In Emacs we load skg-mode and do a title search for the word "has". We should now be in the title search buffer (entitled "skg-title-search"). Unfold the headlin (whatever TAB does in org-mode) and put the cursor on the first character of the substring '][' on the second line.
  Run skg-visit-link. Now we should be at a single-root content view of src, and in the headline there should be a link. Again goto the start of the '][' substring, and skg-visit-link.
  We should now be at a single-root content view of the dest.
* USE IT
...
*** backward links
*** TODO forward links
* I don't mind letting Claude know my .emacs config.
