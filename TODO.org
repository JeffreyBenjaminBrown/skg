...
** ONGOING rust-indent/
*** 1
Welcome! Please read rust-indent/ until you know what's going on. I'm not sure there's any overarching vision to that code, but if there is please try to understand it :)

After that, I would like you to amend it to handle the following case. I want Emacs to be okay with this:
```
...
Currently it instead rewrites that this way:
```
let alias_col_id : Option<NodeId> =
  ( unique_orgnode_child_with_interp (
    tree, node_id, Interp::AliasCol )
    . map_err ( |e| e.to_string() ) ? );
```
** ONGOING Completion is complex.
*** ONGOING reading pub async fn completeAliasCol (
...
*** extract function
And let's extract this, too, into 'prepend_subscribee_col':
