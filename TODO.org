* emacs
** Rename elisp/skg-client.el -> skg-tcp.el
** a metadata viewer/editor
   Transform rules, maybe the only mirror in elisp of orgnode.rs,
   are defined in
...
*** not skg-specific
    org-ancestry
* bug in tools/introspect/call-graph/call-graph.rs
  e.g. update_graph_minus_merges should have three children,
    but only has two
  -- or at least, that's true in in
