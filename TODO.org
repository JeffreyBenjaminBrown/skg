...
** scaffold diff node validation
   (One of these conditions was already implemented since main.)
   Check that each ID has an IDCol parent, no body and no children.
   Check that each IDCol parent has only ID children and no body,
     and has a TrueNode parent.
   Check that each TextChanged has no body or children,
     and has a TrueNode parent.
** not implemented yet
pub fn handle_save_buffer_request (
  ...
  diff_mode_enabled : bool ) {
  // Note: diff_mode_enabled is not yet used, but will be in Phase 5
  let _ = diff_mode_enabled;
* how to prompt
