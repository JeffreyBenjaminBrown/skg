Claude wrote most of this guide.
Jeff edited those parts but has not fact-checked them.

* FIRST: is TypeDB actually the problem?

If =start-servers.sh= fails after a power loss, crash, or forced
reboot -- especially with a *linker* error like =file too short= -- then
TypeDB is fine and nothing in this guide will help. The build artifacts
in =target/= were truncated mid-write. See
=docs/troubleshooting/build-artifacts-corrupted-by-power-loss.org=, and
do not delete any data.

* informal quick fix
If this doesn't do it, read on.
```
...
- `pgrep -f typedb_server_bin` - recommended; matches the actual server binary
- `ps -ef | grep typedb_server_bin` - alternative format

*Caution with bare `grep typedb`*: When running a script whose path
contains "typedb" (e.g., ~bash troubleshooting/typedb/check-typedb-status.sh~),
`ps aux | grep typedb` will match the bash process executing the script,
producing a false positive. Always grep for =typedb_server_bin= specifically.
...
** Note
This issue was fixed in `/home/ubuntu/bash/start-servers.sh` on 2025-10-14. If you're using that script, it should now work correctly. If you're manually starting TypeDB, remember to include the stdin redirection.

* Test Database Accumulation & "Too Many Open Files"

** Quick Fix

If TypeDB panics at startup with "Too many open files" errors (the log
will show a RocksDB error mentioning a specific database's =LOCK= file,
followed by a Rust panic at =main.rs=):
