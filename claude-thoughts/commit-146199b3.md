...
**Entry point:** `instructiontriples_from_the_merges_in_an_orgnode_forest`
...
6. **Return three SaveInstructions** in order:
   ...
   - (acquiree_to_delete, delete action)

**Recognition pattern:** Later phases identify merge instructions by detecting nodes with titles starting with "MERGED-", then treating the next two instructions as the updated_acquirer and acquiree_to_delete.
...
3. **Within SaveInstruction generation:**
   - Instructions must be in order: [acquiree_text_preserver, updated_acquirer, acquiree_to_delete]
...
while i < instructions.len() {
    ...
    if instr.0.title.starts_with("MERGED-") {
        ...
        // Next is acquiree_to_delete
        if i + 2 < instructions.len() {
            acquiree_to_deletes.push(&instructions[i + 2]);
        }
