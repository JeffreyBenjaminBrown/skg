...
** add and use a `folded` field to OrgNode
...
*** update files
    ...
    Keep a list of paths to all files modified or created. For files modified, track whether their text, content and/or links have changed.
...
*** update TypeDB
    ...
    If a file was modified, delete and recreate all of its out-links and contents. Change none of its other ("incoming") links.
*** to Tantivy
    If a file is new, load it as normal.
