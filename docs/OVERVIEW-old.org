#+title: how Skg works
**** THIS FILE IS A COPY. DO NOT EDIT IT.
     The original is part of my knowledge graph,
     at [[id:5c051765-a400-487f-9e31-4fe75a6eeaec][the Skg data model]],
     and not yet online.
* foundation : org-mode
...
*** Each section, subsection, etc. is an 'org node'.
    :PROPERTIES:
...
**** It is often handy to 'fold' an org node.
...
**** 'Levels' (the asterisks) imply containment.
     ...
     and H is its 'child'.
**** Some org nodes are not contained.
     These are 'roots' or 'top-level org nodes'.
*** Folding (hiding contents) makes complex trees readable, navigable.
* terms from org-mode that I use
** fold
   = hide an org node's children (and body), showing only the title
** parent and child
   ...
   In the Skg graph there are no parents or children --
     there are instead explicit named relationships.
...
* the graph
  ...
  :END:
** consists of 'SkgNode's
*** what that means
    Each SkgNode has a 'title' (some text that is always shown),
    a 'body' (some text that can be hidden via 'folding'),
    and some relationships to other nodes.
*** PITFALL: SkgNodes and OrgNodes are different.
    SkgNodes are the fundamental unit of knowledge in a Skg graph.
    ...
    For instances, when a SkgNode N has aliases,
    they are represented as N's descendents in the org view:
```
  * N                                   # true node
  ** its aliases                        # 'scaffold' node
  *** alias 1                           # 'scaffold' node
  *** alias 2                           # 'scaffold' node
  ** the first SkgNode that N contains  # true node
  ** the second SkgNode that N contains # true node
```
As the comments indicate, some of those org nodes
correspond to true skg nodes, while others ('scaffold' nodes)
describe an ancestor -- in this case, the SkgNode N.
...
**** language
     a 'source' node 'textlinks_to' a 'target' node.
**** link representation
...
**** language and definition
     An 'overrider' node 'overrides_view_of' an 'overridden' node.
...
** language
*** 'SkgNode' = an element of the graph
...
** [[https://github.com/JeffreyBenjaminBrown/skg/blob/main/server/types.rs][the orgnode type definitions]] (link to Github)
...
** Some TypeDB data must be calculated from these.
   ...
   and the out-links (for textlinks specifically)
   ...
   And one kind of relationship, textlinks,
