* an early, flabby introduction to skg
** Purpose
...
- Skg's data model and user experience
  ...
  - ["Forks": Subscription and overriding in detail](docs/forks.md)
...
** meh
  The view above only has one root, but views in Skg can have multiple roots.
