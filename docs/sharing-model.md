...
**The gesture.** You do not run a clone command. You make the foreign node *definitive* (editable) with `C-c s d`, edit it, and save. Any real change to a foreign node is read as a request to fork it; the save first shows a read-only confirmation buffer listing what will be forked and waits for your approval (`C-c C-c` to approve, `C-c C-k` to decline). See [forks.md](forks.md) for the walkthrough.
