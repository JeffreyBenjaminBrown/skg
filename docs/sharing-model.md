...
**The gesture.** You do not run a clone command. You make the foreign node *definitive* (editable) with `C-c s d`, edit it, and save. Any real change to a foreign node is read as a request to fork it; the save first shows a confirmation buffer — each clone-to-be over the original it forks — and waits for your approval (`C-c C-c` to approve, `C-c C-k` to decline). The buffer is read-only except each clone's source, which you **must** set with `C-c s s` (it starts as a placeholder with a suggested source noted above it); approving before every clone's source is set is refused. See [forks.md](forks.md) for the walkthrough.
