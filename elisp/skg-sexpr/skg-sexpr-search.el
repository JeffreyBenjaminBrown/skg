...
The display-only herald fields -- semantic (rels ...) facts and
(sourceHerald ⌂:..) inside viewStats -- are
...
Drop node-level (rels ...) display facts
...
now-empty viewStats form). Every other element is returned unchanged."
  (cond
   ((and (consp child) (eq (car child) 'rels))
    nil)
