...
(In case it is helpful, the file
git-diff-view/initial-sketch.org
illustrates the initial concept that birthed this plan.
But wherever that concept contradicts this plan, this plan wins.)
...
See also server/types/orgnode.rs for type definitions.

** source
   A data folder containing .skg files. Each source is expected to be
   a git repository. A user may have multiple sources (e.g., their own
   notes plus subscribed content from other authors). Each source has
   a nickname configured in skgconfig.toml.

** TrueNode vs Scaffold
   See server/types/orgnode.rs

** definitive vs indefinitive
   A TrueNode can be rendered in two ways:
   - Definitive: Shows the node's full content (title, body, children).
   - Indefinitive: Shows only the node's title/header, hiding body and children.
   This is controlled by the ~indefinitive~ boolean field in TrueNode.
   Users can request a "definitive view" to expand an indefinitive node.

** ForestRoot
   A special Scaffold that serves as the invisible root of a "forest".
   In Skg, a buffer's tree structure has ForestRoot at the top;
   level-1 headlines are its children. ForestRoot is never rendered.

** parent_ignores
   A boolean field on TrueNode. When true, saving the buffer will not
   update this node's relationship with its org-parent. The node behaves
   as if it were a tree root, even if it appears nested in the buffer.
...
emulate git diff output. The display interleaves removed and added items
at their respective positions, similar to how git shows deletions (-)
and additions (+) in unified diff format.

Example: [A, B, C] reordered to [C, B, A]
#+begin_example
  A (diff removed-here)   ;; A was here in HEAD, indefinitive
  C (diff new-here)       ;; C moved here, definitive, content shown
  B                       ;; unchanged position
  C (diff removed-here)   ;; C was here in HEAD, indefinitive
  A (diff new-here)       ;; A moved here, definitive
#+end_example

The removed-here instance marks where an item used to be; it's rendered
indefinitive (no content shown) because the item still exists elsewhere.
The new-here instance marks where an item now is; it's rendered
definitive (with content) because this is the item's current location.
...
** Finding changed files
   ...
   2. Use git2's diff functionality to identify which .skg files
      differ between the working directory and HEAD.
      This returns modified, added, AND deleted files in one query,
      without needing to enumerate files on disk first.
...
** Comparison method
   ...
   4. For list comparisons, use a diff algorithm (e.g., LCS/Myers)
      to determine the edit script that produces the
      removed-here/new-here ordering.
      The algorithm identifies which items are unchanged, added, or removed,
      and preserves their relative positions in the output.
...
** Pre-save checks
   ...
   Rationale: Merge commits have multiple parents, making the
   "diff from HEAD" comparison ambiguous (which parent to compare against?).
...
** Special case: root nodes
   If an orgnode has no parent, or has parent_ignores=true,
   or is a direct child of ForestRoot (i.e., a level-1 headline),
...
** removed / removed-here nodes
   Always rendered indefinitive (no content shown).
...
** Definitive view of removed-here node
   Same interface as normal definitive view request.
   Server loads node data from disk (the node exists, just moved).
   Shows the node's current content, not its content at HEAD.

** new / new-here nodes
   Follow normal repeated-node logic:
   - If the node has already been rendered definitively elsewhere,
     this instance is rendered indefinitive.
   - Otherwise, the first new/new-here rendering is definitive
     (with recursive content), and any subsequent renderings
     are indefinitive.
