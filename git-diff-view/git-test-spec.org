...
*** what saving should do
 Try each of these after loading the 'git diff view' described above (in 'test/compare/git diff view/').
**** edits
     If you delete one of the 'removed' nodes and save,
     ...
     If you add a new node as a child somewhere,
       it gets created on disk,
       and its metadata in the returned buffer now says 'diff new'.
