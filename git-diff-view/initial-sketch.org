...
First I'll try to work through an example, and then I'll try to draw principles from it. Suppose the following buffer content is reflected in the last git commit:
...
but the following is a view of the graph since that commit:
  ...
  ** (skg subscribeeCol)
  *** (skg (node (id subee2))) _
  *** (skg (node (id subee3))) _ ;; suppose this already existed
  *** (skg (node (id subee3))) _ ;; suppose this is a new node
  ** (skg aliascol)
  *** (skg alias) a'''
  *** (skg alias) a''
  ** (skg (node (id a3))) _
...
- deleted subee1 and added subee3 as subscribees to a
- deleted a' and inserted a''' as an alias to a
- deleted c and inserted d and e as children of a
