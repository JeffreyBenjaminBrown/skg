...
=path_from_pid_and_source()= (=rust/util.rs:17-27=) dynamically constructs file paths:
...
Modify =path_from_pid_and_source= signature (lines 13-23):
#+begin_src rust
...
fn reconciled_source(
  ...
  primary_id: &ID,  // for error reporting
) -> Result<String, Box<dyn Error>> {
  // Collect all sources from all instances of this ID
  ...
  // CRITICAL: All instances of the same ID MUST have the same source
  // If they don't, that's a validation error
  if sources.is_empty() {
    return Err("No source found in buffer orgnodes for reconciliation".into());
  }
  if sources.len() > 1 {
    return Err(BufferValidationError::InconsistentSources(
      primary_id.clone(), sources).into());
  }

  // All instances have the same source - return it
  Ok(sources.into_iter().next().unwrap())
}
#+end_src

Note: Phase 7 will implement this validation. The current implementation incorrectly
takes the first source without validation (=indefinitives[0].0.source=).
...
- *IMPORTANT*: Current implementation incorrectly takes first source without validation
  (will be fixed in Phase 7 to validate all instances have identical source)
...
** Phase 5: Path Generation and File Writing [COMPLETED]

See =multi-source/phase5-*.md= for detailed analysis and locations.

Key accomplishments:
- Updated =path_from_pid_and_source= to accept source parameter
- Created =pid_and_source_from_id= TypeDB helper
- Updated all call sites (18 locations across 12 files)
- Many functions became async due to TypeDB queries
- Updated tests for async patterns
- Source attribute stored in TypeDB (partial Phase 10)
- All 122 tests passing

Design decision: Single-ID TypeDB queries (not batched). Can optimize later if profiling shows need.
** Phase 6: Source Inheritance During Save [COMPLETED]
...
Implementation (rust/save/buffer_to_orgnodes/add_missing_info.rs):
- Added =parent_source: Option<String>= parameter to =add_missing_info_dfs=
- Created =inherit_source_if_needed= helper function (lines 100-107)
- Function copies parent source to child if child has no source
- Updated initial call to pass None for roots (no parent)
- Updated recursive call to pass current node's source to children
- Extended test_add_missing_info_comprehensive to verify source inheritance
...
1. ✓ Add source consistency validation during reconciliation:
   - *Critical insight*: When multiple orgnodes have the same ID, they MUST have the same source
   - Update =reconciled_source= function in =reconcile_dup_instructions.rs=:
     * Instead of just taking the first source (=indefinitives[0].0.source=),
       collect ALL sources from definer and all indefinitives into a =HashSet=
     * Verify the set is a singleton (contains exactly one unique source)
     * If not singleton, return error with all conflicting sources
   - Add =InconsistentSources(ID, HashSet<String>)= to =BufferValidationError=
   - This validation happens naturally during =reconcile_dup_instructions_for_one_id=,
     which is called AFTER =add_missing_info= (Phase 6) completes source inheritance
   - Error is propagated up and reported alongside other validation errors
   - Add tests for inconsistent source detection with multiple conflicting sources
...
6. ✓ Update =impl Display for BufferValidationError= for all new error types
7. ✓ Add tests for root validation

*Rationale for source consistency validation location*:
- Happens during reconciliation, where duplicate IDs are naturally grouped
- Occurs after source inheritance (Phase 6), ensuring all nodes have sources
- Occurs before any file writes, preventing inconsistent state
- Error propagates through save pipeline and is reported with other validation errors

** Phase 8: Foreign Data Validation [COMPLETED]
1. ✓ Create =rust/save/validate_foreign_nodes.rs= module
2. ✓ Implement =validate_and_filter_foreign_instructions= function
3. ✓ Implement =validate_foreign_merge_instructions= function (for merge validation)
4. ✓ Integrate into save pipeline in =rust/save.rs=
5. ✓ Add tests for:
   - Unmodified foreign nodes (allowed, filtered out)
   - Modified foreign nodes (rejected with ModifiedForeignNode error)
   - Indefinitive foreign nodes (filtered, no append)
   - Owned nodes (unchanged behavior)
   - Deleting foreign nodes (rejected)
   - Creating new foreign nodes (rejected)
   - Mixed owned and foreign nodes
   - Merge with foreign acquirer (rejected)
   - Merge with foreign acquiree (rejected)
   - Merge with both owned nodes (allowed)
...
** Phase 10: TypeDB and Tantivy Integration [PARTIALLY COMPLETED]

TypeDB Integration (COMPLETED):
1. ✓ Updated =create_node= in TypeDB to include source attribute (rust/media/typedb/nodes.rs:134-140)
   - Query now inserts both id and source: =has id "{}", has source "{}"=
   - Required for Phase 5 TypeDB lookups to work
2. ✓ =create_all_nodes= calls work without changes (pass-through)
3. ✓ Tested database initialization with sources (all 122 tests passing)
4. ✓ =pid_and_source_from_id= successfully queries source from TypeDB

Tantivy Integration (PENDING):
1. Add source field to Tantivy schema
2. Update =update_index_with_nodes= to index source
3. Test search with source filtering (future enhancement)
...
- **Path construction changes**: Many call sites to update for =path_from_pid_and_source=
...
- [X] Files are written to correct source directory based on node.source (Phase 5)
- [X] Path generation uses node's source field (Phase 5)
- [X] TypeDB queries can retrieve source for any node ID (Phase 5)
- [X] Nodes inherit source from parent during save (Phase 6)
- [ ] Root nodes without source are rejected with clear error (Phase 7)
...
Data Integrity:
- [X] TypeDB stores source attribute for all nodes (Phase 10 - TypeDB portion)
- [ ] Tantivy indexes source for all documents (Phase 10 - Tantivy portion)
- [X] Duplicate IDs across sources detected at startup
- [X] Source paths validated at startup
- [ ] Orgnodes with same ID must have same source (validated during save in Phase 7)
...
** Source must be set immediately after deserialization
Not serialized to .skg files, must be set after every =read_node()= call. Use wrapper pattern (=read_all_skg_files_from_sources=).

** Duplicate detection: HashMap for O(1) lookup
=HashMap<String, String>= for ID→source mappings. O(N*M) time where N=nodes, M=IDs per node.

** Comprehensive error collection > fail-fast (Phase 3b)
Collect ALL errors before reporting. Users fix everything at once vs iteratively. Better UX for large repos.

** TODO comments track temporary workarounds effectively
"TODO Phase X:" comments help track what needs fixing and why code is written a certain way.

** Sorted output improves testing and debugging (Phase 3b)
Deterministic output enables programmatic comparison. Build expected content, compare strings exactly.

** Make helpers private once API stabilizes (Phase 3b)
Made =read_skg_files= private after Phase 3. Users only see multi-source API.

* Lessons Learned from Phase 5

** TypeDB queries force async propagation
Async propagated through 6+ functions to test code. Plan for full call chain, not just immediate callers.

** Source nickname consistency is critical
Bug: test fixtures used "test" nickname, config used "main". Must be consistent across config, database, and tests.

** Phase dependencies may force reordering
Phase 10 (TypeDB integration) needed early because Phase 5 queries TypeDB. Schema changes needed before queries.

** Test async conversion is mechanical but time-consuming
Pattern: wrap in =run_with_test_db=, add driver parameter, change =?= to =.await?=. Budget time for updates.

* Lessons Learned from Phase 6

** Source inheritance mirrors existing alias inheritance pattern
Same DFS traversal, same parent-to-child parameter passing. Following established patterns made implementation straightforward.

** Helper functions keep DFS traversal clean
=inherit_source_if_needed= isolates inheritance logic, making =add_missing_info_dfs= easier to understand and maintain.

** Option<String> naturally handles parent/child distinction
Roots get =None= (no parent), children get =Some(parent_source)=. Type system enforces the distinction.

** Extended existing test rather than creating new one
Added source metadata to existing =test_add_missing_info_comprehensive= rather than creating separate test. Keeps test suite manageable.

* Open Questions and Future Work

** Performance: Batch TypeDB lookups?
Decision: Deferred until profiling shows need. Current single-ID queries acceptable.

** Testing: Multi-source integration tests?
Resolution (Phase 6): Extended existing tests with source metadata rather than creating multi-source infrastructure. Current single-source test fixtures with explicit source fields are sufficient.

** Design: Source inheritance for non-parent relationships?
Current thinking: Only =contains= relationship inherits source. Other relationships (subscribes_to, overrides_view_of) don't imply ownership.

** Implementation: Source validation timing?
Options: (1) fail later in path_from_pid_and_source, (2) validate in mk_skgnode, (3) validate during tree traversal.
Consider for Phase 7 validation.

** Design: Source renaming support?
Recommendation: Document as unsupported. Would require config update + TypeDB rebuild + org buffer updates. Add migration tool if needed later.

** Infrastructure: Multi-source test infrastructure?
Resolution (Phase 6): Not needed. Extended existing single-source tests with explicit source metadata. Current =run_with_test_db= infrastructure sufficient.
...
** 6. How to determine source when reading individual nodes (Phase 5) ✓ RESOLVED

Decision: Use both approaches depending on context

Analysis revealed three categories of `path_from_pid_and_source` call sites:
1. **Category A (3 sites):** Caller has the node object → add `source` parameter to `path_from_pid_and_source`
2. **Category B (8 sites):** Caller only has ID → create TypeDB helper to query source
3. **Category C (1 site):** Already computed source → use the existing value

Implementation approach:
- Modify `path_from_pid_and_source` to accept `source` parameter
- Create `pid_and_source_from_id` helper (or wrapper functions) to query TypeDB
- Update call sites according to their category

Detailed analysis: See =multi-source/phase5-analysis.md=

Rationale:
- Different call sites have different contexts (some have node, some only have ID)
- TypeDB already stores source attribute, can query it efficiently
- Avoids fragile path-parsing approaches
- Clean separation: path construction vs source lookup

** 7. Should source lookups be batched like pids_from_ids? ✓ RESOLVED

Decision: Start with single-ID lookups, add batching if profiling shows need

Analysis revealed:
- Most Category B call sites (6 of 8) process single IDs sequentially
- Only 1 site reads 2 nodes in quick succession (merge operations)
- Batching infrastructure exists (=pids_from_ids=) showing the pattern to follow
- Most call sites not easily batchable without major refactoring

Implementation approach:
- Create simple =pid_and_source_from_id= for single lookups
- Measure performance during real usage
- Add =pids_and_sources_from_ids= batch function later if justified

Detailed analysis: See =multi-source/phase5-batching-analysis.md=

Rationale:
- Avoid premature optimization
- Match current usage patterns (mostly one-off lookups)
- Can optimize later with minimal API changes if measurements justify it
...
1. **Phase 5: Path Generation and File Writing**
   - Update =path_from_pid_and_source= signature to accept source parameter
   - Update all 11+ call sites of =path_from_pid_and_source=
