...
See =multi-source/phase5-analysis.md= for detailed call site analysis.
See =multi-source/phase5-batching-analysis.md= for performance considerations.

Key finding: Call sites fall into 3 categories:
1. **3 sites** already have node.source → add parameter
2. **8 sites** only have ID → need TypeDB helper to get source
3. **1 site** already computed source → use it

Performance decision: Start with single-ID lookups, add batching later if needed
- Most Category B sites process one ID at a time
- Batching infrastructure exists (=pids_from_ids=) if optimization needed
- Premature optimization avoided

Implementation steps:
1. Update =path_from_pid= signature to accept source parameter
2. Create TypeDB helper: =pid_and_source_from_id= (single-ID lookup)
3. Update Category A call sites (3): pass node.source
4. Update Category B call sites (8): use TypeDB helper
5. Update Category C call site (1): use reconciled_source_value
6. Modify =write_all_nodes_to_fs= to use source-specific paths
7. Test writing to multiple source directories

Future optimization: Add =pids_and_sources_from_ids= batch function if profiling shows need
...
Analysis revealed three categories of `path_from_pid` call sites:
1. **Category A (3 sites):** Caller has the node object → add `source` parameter to `path_from_pid`
...
- Modify `path_from_pid` to accept `source` parameter
