...
M.overPrivateText_retry = nil
M.overPrivateText_challenged = false
...
function M.register_overPrivateText_confirmation (retry, unfired_response_type)
  M.overPrivateText_retry = nil
  M.overPrivateText_challenged = false
  state.register_response_handler('overPrivateText-telescope-confirmation',
    function (_payload_text, response)
      ...
      M.overPrivateText_challenged = true
      ...
      if vim.fn.confirm(prompt, '&Include\n&Decline', 2) == 1 then
        M.overPrivateText_retry = function () retry(pids) end
      end
    end, false)
end
...
function M.register_rerender_stream_handlers ()
  state.register_response_handler('rerender-lock',
    function (_payload_text, response)
      if not M.overPrivateText_challenged then
        state.response_handler_map['overPrivateText-telescope-confirmation'] = nil
      end
      ...
      local retry = M.overPrivateText_challenged and M.overPrivateText_retry or nil
      M.overPrivateText_retry = nil
      M.overPrivateText_challenged = false
