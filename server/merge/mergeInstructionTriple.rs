...
use crate::types::views_state::nodecomplete_from_in_rust_graph_or_disk_async;
...
pub async fn neighbor_savenodes_for_merges (
  ...
  for pid in &to_load {
    let node : NodeComplete =
      nodecomplete_from_in_rust_graph_or_disk_async (config, driver, pid) . await ?;
...
async fn optmerge_from_viewnode (
  ...
  let acquirer_from_disk : NodeComplete =
    nodecomplete_from_in_rust_graph_or_disk_async (
