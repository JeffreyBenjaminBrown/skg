...
pub struct SkgNode {
  // There is a 1-to-1 correspondence between SkgNodes and actual .skg files -- a file can be read to a SkgNode, and a SkgNode can be written to a file. The files are the only permanent data. SkgNode is the format used to initialize the Neo4j and Tantivy databases.
  // Tantivy will receive some of this data, and Neo4j some other subset. Tantivy associates IDs with titles. Neo4j represents all the connections between nodes. At least one field, `body`, is known to neither database; it is instead read directly from the files on disk when Rust builds a document for Emacs.
  ...
  pub body: Option<String>, // Unknown to both Tantivy & Neo4j. The body is all text (if any) between the preceding org headline, to which it belongs, and the next (if there is a next).
