...
/// Convert an UncheckedOrgNode tree to a checked OrgNode tree,
/// filling in placeholder values for missing id/source.
/// Used by test comparison functions to reuse the checked comparison logic.
pub fn unchecked_to_checked_tree_with_fake_id_and_source(
  unchecked: &Tree<UncheckedOrgNode>
) -> Tree<OrgNode> {
  fn convert_node(u: &UncheckedOrgNode) -> OrgNode {
    OrgNode {
      focused: u.focused,
      folded: u.folded,
      kind: match &u.kind {
        UncheckedOrgNodeKind::True(t) => OrgNodeKind::True(TrueNode {
          title: t.title.clone(),
          body: t.body.clone(),
          id: t.id_opt.clone()
            .unwrap_or_else(|| ID::from("ignored_id")),
          source: t.source_opt.clone()
            .unwrap_or_else(|| "ignored_source".to_string()),
          parent_ignores: t.parent_ignores,
          indefinitive: t.indefinitive,
          graphStats: t.graphStats.clone(),
          viewStats: t.viewStats.clone(),
          edit_request: t.edit_request.clone(),
          view_requests: t.view_requests.clone(),
          diff: t.diff.clone(),
        }),
        UncheckedOrgNodeKind::Scaff(s) =>
          OrgNodeKind::Scaff(s.clone()),
      },
    }
  }
  ...
  fn convert_children(
    ...
    for unchecked_child_id in child_ids {
      ...
      let checked_child :
        OrgNode =
        convert_node(unchecked_child);

      let checked_child_id :
        NodeId =
        { let mut parent_mut :
           NodeMut<OrgNode> =
           checked_tree.get_mut(checked_id).unwrap();
  ...
  let root_unchecked :
    &UncheckedOrgNode =
    unchecked.root().value();
  let root_checked :
    OrgNode =
    convert_node(root_unchecked);
  let mut checked :
    Tree<OrgNode> =
    Tree::new(root_checked);

  let unchecked_root_id :
    NodeId =
    unchecked.root().id();
  let checked_root_id :
    NodeId =
