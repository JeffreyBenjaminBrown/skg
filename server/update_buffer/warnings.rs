...
/// ColRepairs render one string per (folder, owner) pair, in
...
pub fn render_completion_warnings (
  ...
  { // the ColRepairs, batched
    ...
    for w in warnings {
      let CompletionWarning::FolderRepair { folder, owner, .. } = w;
      let key : (PartnerFolder, ID) =
        ( *folder, owner . clone () );
      if ! group_order . contains (&key) {
        group_order . push (key); } }
    for (folder, owner) in &group_order {
      let mut segments : Vec<String> = Vec::new ();
      let mut any_restored : bool = false;
      for w in warnings {
        let CompletionWarning::FolderRepair {
          folder : w_folder, owner : w_owner, repair, children } = w;
        if w_folder != folder || w_owner != owner { continue; }
