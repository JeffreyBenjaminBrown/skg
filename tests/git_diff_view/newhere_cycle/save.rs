...
{
  let test_name = "skg-test-git-diff-view-newhere-cycle-save";
  ...
  block_on(async {
    ...
    cleanup_test_stores(test_name, Some(Path::new (tantivy_folder))) . await?;
    Ok(())
  })
}
...
fn test_newhere_cycle_survives_save_staged()
  -> Result<(), Box<dyn Error>>
{
  let test_name = "skg-test-git-diff-view-newhere-cycle-save-staged";
