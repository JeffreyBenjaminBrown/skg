...
backup_and_reset_test_data() {
    ...
    find "$TEST_DIR/data/owned/skg-data" -name "*.skg" ! -name "1.skg" -delete 2>/dev/null || true
    echo "✓ Cleaned up existing UUID files"

    # Backup original 1.skg file
    if [ -f "$TEST_DIR/data/owned/skg-data/1.skg" ]; then
        BACKUP_FILE="$TEST_DIR/data/owned/skg-data/1.skg.backup.$(date +%s)"
        cp "$TEST_DIR/data/owned/skg-data/1.skg" "$BACKUP_FILE"
...
cleanup_test_data() {
    ...
    UUID_FILES=$(find "$TEST_DIR/data/owned/skg-data" -name "*.skg" ! -name "1.skg" 2>/dev/null || true)
    ...
    BACKUP_FILE=$(find "$TEST_DIR/data/owned/skg-data" -name "1.skg.backup.*" | head -1)
    if [ -n "$BACKUP_FILE" ] && [ -f "$BACKUP_FILE" ]; then
        cp "$BACKUP_FILE" "$TEST_DIR/data/owned/skg-data/1.skg"
        echo "✓ Restored 1.skg from backup"
    fi
    find "$TEST_DIR/data/owned/skg-data" -name "1.skg.backup.*" -delete
}
