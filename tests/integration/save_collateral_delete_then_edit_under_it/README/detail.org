...
From that buffer we remove 'writeProtected' from 11's metadata,
...
2. Editing under a =DeletedNode=: a subsequent save from the
   ...
   picks up the change.
3. Re-saving a buffer containing =Deleted= / =DeletedScaff= nodes:
   they round-trip through parse → completion → render unchanged,
   and a new node created under a =DeletedScaff= gets a UUID.
...
Five =.skg= files under =data/owned/skg-data/=:
...
1. Request =skg-request-single-root-content-view-from-id "1"=.
   The server renders all scaffolding for node 11 (subscribeeFolder,
...
#+end_example
   ...
   the aliasFolder and alias for node 11).

** Expected initial structure (titles)

Scaffold rendering uses fixed titles ("it subscribes to these",
=server/types/viewnode.rs=. Non-vognode folder headlines no
longer carry explanatory titles; the client herald rules explain them.
The expected =headline-titles= output is approximately:
...
response. Since 11 is writeProtected it generates no save
...
Edit buffer 2:
- Remove =writeProtected= from 11's metadata
...
hiddenOutsideOfSubscribeeFolder) become =DeletedScaff=. The
aliasFolder also becomes =DeletedScaff=; its leaf child
(the =alias= scaffold) becomes a childless =DeletedScaff=
which the postorder pass detaches. So aliasFolder and its
alias disappear entirely.

subee is a TrueNode under the (now =DeletedScaff=)
subscribeeFolder. =complete_viewtree= completes it normally,
...
((1 . "1")
 (2 . "11")           ;; DeletedNode
 (3 . "")             ;; DeletedScaff (was subscribeeFolder)
 (4 . "subee")        ;; TrueNode, completed
 (5 . "subee-1")      ;; child from completion
 (5 . "")             ;; DeletedScaff (was hiddenInSubscribeeFolder)
 (6 . "subee-1")      ;; hidden node preserved
 (4 . "")             ;; DeletedScaff (was hiddenOutsideOfSubscribeeFolder)
...
is detached). The aliasFolder scaffold becomes DeletedScaff;
its leaf child (the =alias= scaffold) also becomes childless
DeletedScaff and IS detached. So aliasFolder and its alias
disappear entirely.
...
* Phase 5: Add new-root under deletedScaff in buffer 1

Insert =new-root= as a sibling of =subee= under the depth-3
=DeletedScaff= (formerly =subscribeeFolder=) in buffer 1. Source
metadata must be supplied explicitly (=(source main)=) because
=DeletedScaff= has no source to inherit from.

This exercises:
- =Deleted= / =DeletedScaff= round-tripping through the save
  pipeline (parse → completion → render) without disruption.
- Creating a new node under a =DeletedScaff=: the node gets a
  UUID and is saved to disk, but no parent claims it in its
  =contains= (the =DeletedScaff= generates no save instructions).

** Expected buffer 1 after save

Same as before, but =new-root= appears at depth 4 with a UUID.
...
((1 . "1")
 (2 . "11")           ;; DeletedNode
 (3 . "")             ;; DeletedScaff
 (4 . "subee")        ;; TrueNode (writeProtected)
 (4 . "new-root")     ;; new TrueNode (now has UUID)
 (4 . "")             ;; DeletedScaff
 (5 . "also-hidden")  ;; hidden node preserved
 (1 . "subee")        ;; standalone root, unchanged
 (2 . "subee-1")
 (2 . "subee-2"))
#+end_example
...
to =home_from_disk(id, config)= (defined in
...
=nodecomplete_from_map_or_disk=) takes =&SourceName=, so use
=&child_source= there.

Import =home_from_disk= at the top of the file from
=crate::types::phantom::home_from_disk=.
...
| =data/owned/skg-data/1.skg=             | Fixture: node 1               |
| =data/owned/skg-data/11.skg=            | Fixture: node 11              |
| =data/owned/skg-data/subee.skg=         | Fixture: node subee           |
| =data/owned/skg-data/subee-1.skg=       | Fixture: node subee-1         |
| =data/owned/skg-data/also-hidden.skg=   | Fixture: node also-hidden     |
...
metadata sexp itself — e.g., that it contains =(deleted ...)=
rather than =(node ...)=.
...
with NO title. The scaffolds they replaced (subscribeeFolder, etc.)
...
=* (writeProtected) 11= means a write-protected TrueNode, etc. None of these
...
- =(writeProtected) 11= → =(skg (node (id 11) (source main) writeProtected)) 11=
...
Confirmed from code: in =complete_postorder_with_limited_recursion=
...
Fix: add a =home_from_disk= fallback to
...
1. Remove =writeProtected= from 11's metadata
2. Add =(editRequest delete)= to 11's metadata

=skg-edit-metadata-at-point= merges sexp structures. It can ADD
fields but it's unclear if it can REMOVE =writeProtected= (a bare
atom, not a key-value pair). We may need to use =(DELETE writeProtected)=
syntax (like =skg-remove-focused-marker= uses =(DELETE focused)=).

So the two calls would be:
1. =(skg-edit-metadata-at-point '(skg (DELETE writeProtected)))=
...
The server's save pipeline includes filesystem writes, graph publication, and
Tantivy reindex, merge, and collateral completion. The containerward
