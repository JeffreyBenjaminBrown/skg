...
# Launching from $PROJECT_ROOT keeps the config path relative to that root,
# which is what makes config.data_root relative -- the condition under which
# the path-duplication bug manifests.
cd "$PROJECT_ROOT"
