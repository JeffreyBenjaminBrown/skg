...
fn test_find_in_metadata_collection() {
  ...
  // Test finding ID
  let id = find_id_in_metadata_collection(&metadata);
  assert_eq!(id, Some(ID("test123".to_string())));

  // Test generic function with different accessors
  let has_cycle = find_in_metadata_collection(&metadata, |item| {
    if item.is_cycle() { Some(()) } else { None }
  });
  assert_eq!(has_cycle, Some(()));

  let type_val = find_in_metadata_collection(&metadata, |item| {
    item.get_type().cloned()
  });
  assert_eq!(type_val, Some(RelToOrgParent::Container));

  // Test with empty collection
  let empty: Vec<MetadataItem> = vec![];
  let no_id = find_id_in_metadata_collection(&empty);
