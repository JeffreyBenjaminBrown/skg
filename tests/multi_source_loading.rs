...
fn test_load_from_single_source() {
  ...
  set_source_retagging_relSources ( &mut node, &SourceName::from ("main") );
...
fn test_load_from_multiple_sources() {
  ...
  set_source_retagging_relSources ( &mut node1, &SourceName::from ("main") );
  ...
  set_source_retagging_relSources ( &mut node2, &SourceName::from ("main") );
...
fn test_telescope_is_not_a_conflict_but_two_pids_are() {
  ...
  { // What REMAINS a conflict: one id claimed by two distinct pids
    ...
    set_source_retagging_relSources ( &mut node_a, &SourceName::from ("main") );
    ...
    set_source_retagging_relSources ( &mut node_b, &SourceName::from ("shared") );
    let result : IoResult<()> =
      error_unless_each_id_names_one_node (
...
fn test_source_field_set_correctly() {
  ...
  set_source_retagging_relSources ( &mut node_a, &SourceName::from ("source_a") );
  write_nodecomplete_to_source(&node_a, &config) . unwrap();

  let mut node_b : NodeComplete = empty_node_complete();
  node_b . pid = ID::new ("node_b");
  node_b . title = "Node B" . to_string();
  set_source_retagging_relSources ( &mut node_b, &SourceName::from ("source_b") );
...
fn test_unreadable_files_creates_org_file() {
  ...
  set_source_retagging_relSources ( &mut node, &SourceName::from ("source_good") );
...
/// arises from a skg save (every relSource is clamped to at least the
...
fn a_write_refuses_a_foreign_home_and_a_title_hoist() {
  ...
  { // FOREIGN HOME: refused, rather than silently dropping the title.
    ...
    set_source_retagging_relSources (
...
fn ordinary_writers_refuse_body_only_hoists_and_preflight_the_batch() {
  ...
  invalid . contains . push (
    skg::types::misc::RelPartner::at_relSource (
