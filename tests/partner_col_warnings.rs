...
async fn readonly_col_repairs_warn_impl (
  ...
  let response : SaveResponse =
    save_buffer (&edited, config, fixture_graph, tantivy, &graph) . await ?;
  ...
  { // A BODY on a scaffold still aborts the save (Body_of_Scaffold).
    ...
    let result =
      save_buffer (&with_body, config, fixture_graph, tantivy, &graph) . await;
...
";
      let result : Result<SaveResponse, Box<dyn Error>> =
        save_buffer (buffer, config, fixture_graph, tantivy, &graph) . await;
