...
** TODO how to redo buffer update
*** refs
    server/update_buffer/complete_parent_first/truenode.rs
*** notes and complications
**** We now correct, not regenerate, aliases and other 'regenerables'.
...
**** subtree_has_focus
**** move_child_to_end
*** TODO do these, in serial
**** DONE DFS preorder (parent-first) completion
***** TrueNode
****** apply make_indef_if_repeat_then_extend_defmap to it
****** if it's definitive
******* if it's not a subscribee
...
******* if it's a subscribee
Its target child content list depends on hidden nodes. Do everything in 'if it's not a subscribee' except with a different content list.
...
**** DONE compute GraphNodeStats
     The function to do this is named
       set_graphnodestats_in_forest
**** ONGOING compute ViewNodeStats
     This currently happens in at least two places.
       detect_and_mark_cycle is one of them.
       set_metadata_relationships_in_node_recursive is another.
       I *think* there are only those two places.
     They should be collected into a single function,
       set_viewnodestats_in_forest
     Don't worry about plugging that new function in;
       it won't be reachable from main yet.
     Please put the new function in a new file,
       server/update_buffer.rs
**** TODO wire it together
I would like a function 'complete_viewtree' at server/update_buffer.rs. If you look in server/update_buffer/ you'll find code it will need.

'complete_viewtree_node' should perform two recursive traversals of the view tree: the first dfs preorder (parents-first), and the second dfs postorder (children first). The functions to use in each case can be found in the two folders 'complete_child_first' and 'complete_parent_first'. There will be some switch statement delegating to these functions. In the parent-first pass, if it's not a truenode or a subscribeecol, don't do anything (but do recurse into its children, regardless).

In the child-first pass, if it's a ForestRoot, TextChanged, Alias or ID, do nothing, but add a comment that in each case we depend for the node's correctness on its parent having been processed properly.

Then, 'complete_viewtree' should be defined using 'do_everywhere_in_tree'.
** cleanup
