...
*** ONGOING Subscribee is a TrueNode case
*** TODO git diff view affects SubscribeeCol's children
Compute a diff list, like in complete_truenode_preorder,
for the subscribeecol's children.
This should be whatever its parent subscribes to (new),
diffed against whatever it used to subscribe to (old).
