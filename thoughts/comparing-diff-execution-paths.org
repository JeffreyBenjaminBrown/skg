...
*** 4. IDFolder scaffold with ID children (INSERTS nodes)

If =node_changes.ids_diff= contains any non-Unchanged entry:
- PREPENDS a =Scaffold::IDFolder= child.
...
- Does NOT check whether an IDFolder already exists.
...
- Does not handle AliasFolder diffs (no =Scaffold::Alias { diff }= children).
- Does not handle subscribee diffs (SubscribeeFolder, HiddenInSubscribeeFolder,
...
- For subscribee nodes (children of a SubscribeeFolder):
...
- If =ids_diff= has non-Unchanged entries: INSERTS empty
  =Scaffold::IDFolder= (only if absent).
- If =aliases_diff= has non-Unchanged entries: INSERTS empty
  =Scaffold::AliasFolder= (only if absent).

The IDFolder and AliasFolder are inserted EMPTY here. They get populated
...
** Preorder pass: =complete_subscribee_folder_preorder=

File: =update_buffer/complete_parent_first/subscribee_folder.rs=

Called for every =Scaffold::SubscribeeFolder=.
...
** Postorder pass: =completeIDFolder=

File: =update_buffer/complete_child_first/id_folder.rs=

Called for every =Scaffold::IDFolder=. Populates it with
...
** Postorder pass: =completeAliasFolder=

File: =update_buffer/complete_child_first/aliasFolder.rs=

NOT diff-aware. Creates =Scaffold::Alias { text, diff: None }=
children from worktree aliases. Does not consult =source_diffs= at
all. (The preorder pass creates the empty AliasFolder scaffold when
aliases have changed, but the postorder populates it without diff
markers.)

*** TODO It should be diff-aware.

** Postorder pass: =complete_hiddeninsubscribee_folder=

File: =update_buffer/complete_child_first/hiddenInSubscribee_folder.rs=
...
** Postorder pass: =complete_hiddenoutsideofsubscribeefolder=

File: =update_buffer/complete_child_first/hiddenoutsideof_subscribeefolder.rs=
...
|--------------------------------------+---------------+----------------|
| Set =t.diff = NotInGit=              | yes           | yes            |
| Set =t.diff = New= (file added)      | yes           | yes            |
| Set =t.diff = Removed= (file del.)   | yes           | yes            |
| Set =t.diff = NewHere= on children   | yes           | yes            |
| Insert phantom content children      | yes           | yes            |
| Insert =TextChanged= scaffold        | yes           | yes            |
| Insert =IDFolder= scaffold              | yes           | yes            |
| Populate IDFolder with ID children      | yes           | yes            |
| Insert =AliasFolder= scaffold           | no            | yes            |
| Populate AliasFolder with Alias{diff}   | no            | yes            |
...
** IDFolder scaffold + ID children

Both paths insert an IDFolder. diff.rs prepends it already populated
with ID children. complete's preorder inserts an empty IDFolder (checked
for uniqueness), then the postorder populates it. If complete runs
first, diff.rs will add a second, fully-populated IDFolder.
