...
*Learnings:*
- Test uses =run_with_test_db= helper which requires Box::pin closure
- Need fixtures directory even if empty for test DB setup
...
1. Rewrite =complete_or_restore_each_node_in_branch_v2= to use:
   - =add_v_to_map_if_absent= instead of =ensure_skgnode=
