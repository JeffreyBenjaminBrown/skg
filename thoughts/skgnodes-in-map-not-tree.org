...
** Switch Entry Point to v2

In =server/serve/handlers/save_buffer.rs=:
- Change =handle_save_buffer_request= to call =update_from_and_rerender_buffer_v2=
- Or add flag/config to choose between old and new implementations
- Test with real usage to verify behavior matches
