...
** Chunk 1: Add Helper Functions (Non-Breaking)

*Goal:* Introduce new functions that work with maps, without changing any existing code.

*Changes:*
1. Add to =server/types/tree.rs= (or new =server/types/skgnode_map.rs=):
   #+begin_src rust
...
*Tests:* Write unit tests for =skgnode_for_orgnode= covering:
- TrueNode with ID in map → returns Some
- TrueNode with ID not in map → returns None
- Scaffold → returns None

*Verification:* =cargo test= passes, no existing code changed.

** Chunk 2: Add Map-Building Helper (Non-Breaking)

*Goal:* Create function to build SkgNodeMap from SaveInstructions.

*Changes:*
1. Add to same file as Chunk 1:
   #+begin_src rust
   /// Build a map from SaveInstructions.
...
*Tests:* Unit test verifying the map is built correctly from sample SaveInstructions.

*Verification:* =cargo test= passes.

** Chunk 3: Refactor One Leaf Utility Function
...
1. Add new version alongside old:
   ...
   #+end_src
2. Keep old =get_pid_in_pairtree= for now

*Tests:* Copy existing tests for =get_pid_in_pairtree=, adapt for new function.

*Verification:* Both old and new functions work, tests pass.

** Chunk 4: Refactor =ensure_skgnode= to Work with Map

*Goal:* Create map-based version of =ensure_skgnode=.

*Changes:*
1. Add new function (keep old one):
   #+begin_src rust
   pub async fn ensure_skgnode_in_map(
     node_id: &ID,
     map: &mut SkgNodeMap,
...
*Tests:* Test that it:
- Fetches and adds missing SkgNode
- Doesn't re-fetch if already present

*Verification:* Tests pass, old code unchanged.

** Chunk 5: Create Dual-Signature Version of =complete_or_restore_each_node_in_branch=

*Goal:* Add overload that takes (Tree, Map) instead of PairTree, delegate to old version temporarily.

*Changes:*
1. Add new function that converts inputs to PairTree, calls old function, then extracts results back:
   #+begin_src rust
   pub async fn complete_or_restore_each_node_in_branch_v2(
     ...
     // Temporarily: build PairTree from tree+map
     let mut pairtree = /* convert */;
     complete_or_restore_each_node_in_branch(&mut pairtree, /* ... */).await?;
     // Extract changes back to tree and map
     Ok(())
   }
   #+end_src

*Tests:* Verify new function produces same results as old.

*Verification:* Both versions work identically.

** Chunk 6: Refactor Internals of =complete_or_restore_each_node_in_branch_v2=

*Goal:* Reimplement v2 to work directly with tree+map, not via conversion.

*Changes:*
1. Rewrite =complete_or_restore_each_node_in_branch_v2= to use:
   - =ensure_skgnode_in_map= instead of =ensure_skgnode=
   - =get_pid_in_tree= instead of =get_pid_in_pairtree=
   - Direct tree/map access

*Tests:* Verify behavior unchanged from Chunk 5.

*Verification:* Tests pass, especially repeat and cycle tests.

** Chunk 7: Refactor Sub-Functions of Completion (One at a Time)

*Goal:* Convert =clobberIndefinitiveOrgnode= to use tree+map.

*Changes:*
1. Add =clobberIndefinitiveOrgnode_v2= working with tree+map
2. Update =complete_or_restore_each_node_in_branch_v2= to use it

*Tests:* Verify indefinitive nodes are clobbered correctly.
...
- Other completion helpers (one per chunk)
