...
** 7.1 Migrate rendering subsystem
   Files: server/org_to_text.rs

   - org_to_text.rs already has new_orgnode_to_text() functions
   - Update callers to use new functions (convert OrgNode -> NewOrgNode)
   - Eventually remove old orgnode_to_text() when unused
...
** 7.2 Migrate tree construction subsystem
   ...
   - Update to build NewOrgNode internally
   - Convert to OrgNode at boundary (for PairTree compatibility)
   - Test after each file

** 7.3 Switch PairTree to use NewOrgNode
   Files: server/types/tree.rs, server/types/tree/orgnode_skgnode.rs

   This is the central change:
   - Change NodePair.orgnode from OrgNode to NewOrgNode
   - Or: Replace NodePair with NewNodePair entirely
   - Update orgnode_skgnode.rs helper functions
   - Update all callers (should be ready from 7.2)
   - Test thoroughly
...
** Notes on 7.3 (the tricky step)
   Changing NodePair.orgnode to NewOrgNode is the "big bang" moment.
   All code that accesses orgnode field must change simultaneously.

   Alternative approach (if 7.3 is too risky):
   - Add a new field: NodePair { orgnode: OrgNode, new_orgnode: Option<NewOrgNode> }
   - Migrate callers one by one to use new_orgnode
   - When all use new_orgnode, remove orgnode field
   - This is slower but safer

   Decision: Choose based on confidence after completing 7.1-7.2
