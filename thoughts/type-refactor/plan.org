...
** 8.0 Complete type migration (if desired)
   The dual-field approach is stable but maintains two type systems.
   To fully migrate to new types only:
   - Migrate parsing to produce NewOrgNode directly (or Tree<NewOrgNode>)
   - Remove orgnode field from NodePair
   - Rename NewOrgNode → OrgNode, new_orgnode → orgnode
   - Remove conversion functions (from_old_orgnode, to_old_orgnode)
   - Remove old types (OrgNode, Interp, OrgnodeCode, OrgnodeMetadata)
   This is substantial work with limited benefit beyond cleaner types.
