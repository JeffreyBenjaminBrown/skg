...
* Phase 3: Migrate tree construction (output path continued)
  These functions build OrgNode trees from disk/TypeDB data.
** 3.1 Update to_org/util.rs
   - forest_root_pair() should create NewOrgNode with Scaffold::ForestRoot
   - build_node_branch_minus_content() and helpers
** 3.2 Update to_org/expand/aliases.rs
   - Creates AliasCol and Alias scaffolds
** 3.3 Update to_org/complete/sharing.rs
   - Creates SubscribeeCol, HiddenInSubscribeeCol, HiddenOutsideOfSubscribeeCol
   - Creates Subscribee and HiddenFromSubscribees TrueNodes
** 3.4 Update to_org/complete/contents.rs
   - Main content tree building
** 3.5 Update to_org/complete/aliascol.rs
   - AliasCol completion logic
** 3.6 Update to_org/expand/backpath.rs
   - Creates ParentIgnores TrueNodes
** 3.7 Update to_org/expand/definitive.rs
   - View request handling
** 3.8 Update to_org/render/initial_bfs.rs
   - BFS rendering, sets indefinitive on truncated nodes
** 3.9 Run all tests after each sub-step

* Phase 4: Migrate parsing (input path)
  Parsing is trickier because it builds OrgNodes from user text.
** 4.1 Update serve/parse_metadata_sexp.rs
   - Parse sexp metadata into new structures
   - Determine TrueNode vs Scaffold from interp field
** 4.2 Update from_text/buffer_to_orgnodes/uninterpreted.rs
   - Initial parsing creates OrgNodes
   - Needs to decide TrueNode vs Scaffold based on metadata
** 4.3 Update from_text/buffer_to_orgnodes/add_missing_info.rs
   - Adds source inheritance, etc.
** 4.4 Update from_text/buffer_to_orgnodes/validate_tree.rs
   - Validation logic
   - Many checks become unnecessary (type system enforces them)
   - Some checks get simpler (e.g., "Scaffold should not have ID")
** 4.5 Run tests after each sub-step

* Phase 5: Migrate save instructions (input path continued)
** 5.1 Update from_text/orgnodes_to_instructions/to_naive_instructions.rs
   - Converting OrgNode tree to save instructions
   - Only TrueNodes generate save instructions
** 5.2 Update from_text/orgnodes_to_instructions/reconcile_same_id_instructions.rs
** 5.3 Update from_text/validate_foreign_nodes.rs
** 5.4 Run tests

* Phase 6: Migrate remaining code
** 6.1 Update merge/validate_merge.rs
** 6.2 Update merge/mergeInstructionTriple.rs
** 6.3 Update types/tree/orgnode_skgnode.rs
   - Helper functions for trees
** 6.4 Update serve/handlers/*.rs
** 6.5 Update test_utils.rs
** 6.6 Run all tests
