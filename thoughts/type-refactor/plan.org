...
** 8.5 Remove orgnode field from NodePair
   Once all code uses new_orgnode:
   - Change NodePair to only have: { mskgnode, new_orgnode }
   - Update all NodePair constructions
   - Remove from_old_orgnode syncing

** 8.6 Rename types to final names
   - NewOrgNode → OrgNode
   - new_orgnode → orgnode
   - new_orgnode_to_text → orgnode_to_text
   - orgnode_new() → orgnode()

** 8.7 Remove old types and conversion functions
