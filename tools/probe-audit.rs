 ...
 * in the freshly-rebuilt in-Rust memory.
 *
 * Checks three pids:
 *   7be4b170 — was flagged 'contains contained mismatch':
 *              memory {1ff18102}, typedb {d923d1bd, 1ff18102}
 *   aba033b6 — was flagged 'textlinks_to source mismatch':
 *              memory {ee08b16a}, typedb {}
 *   bb78d57e — was flagged 'textlinks_to source mismatch' yesterday
 *              (should no longer be in memory post-rebuild; the .skg
 *              file is only in public-1-mangled/)
 *
 * Prints both memory's and TypeDB's answer for each, so we can see
...
async fn main () -> Result<(), Box<dyn std::error::Error>> {
  ...
  for pid_str in [
    ...
    println! ("\n=== {} ===", pid_str);
    let in_memory : bool = snap . nodes . contains_key (&pid);
    println! ("  in memory.nodes?          {}", in_memory);
    if ! in_memory {
      println! ("  (skipping — not a node in this graph)");
      continue; }
    let mms = audit_one_node (&snap, &tx, &pid) . await ?;
    if mms . is_empty () {
      println! ("  NO mismatches — memory and TypeDB agree.");
    } else {
      for m in &mms {
        println! ("  MISMATCH: {} {} \n    memory: {:?}\n    typedb: {:?}",
                  m . relation, m . role, m . memory, m . typedb); } } }
  Ok (( )) }
